...
var calculations_2 = require("../../earth/calculations");
var calculations_3 = require("../../sun/calculations");
...
function _getSumR(T) {
    ...
    var Msun = calculations_3.sunCalc.getMeanAnomaly(T);
    var Mmoon = getMeanAnomaly(T);
    var F = getArgumentOfLatitude(T);
    var E = 1 - 0.002516 * T - 0.0000074 * Math.pow(T, 2);
    var sumR = 0;
    calculations_1.MOON_ARGUMENTS_LR.forEach(function (args) {
        var argD = args[0];
        var argMsun = args[1];
        var argMmoon = args[2];
        var argF = args[3];
        var argSumR = args[5];
        var tmpSumR = Math.cos((0, angleCalc_1.deg2rad)(argD * D + argMsun * Msun + argMmoon * Mmoon + argF * F));
        switch (argMsun) {
            case 1:
            case -1:
...
function _getSumL(T) {
    ...
    var E = 1 - 0.002516 * T - 0.0000074 * Math.pow(T, 2);
    var sumL = 3958 * Math.sin((0, angleCalc_1.deg2rad)(A1))
    ...
    calculations_1.MOON_ARGUMENTS_LR.forEach(function (args) {
        ...
        var argF = args[3];
        var argSumL = args[4];
        var tmpSumL = Math.sin((0, angleCalc_1.deg2rad)(argD * D + argMsun * Msun + argMmoon * Mmoon + argF * F));
        switch (argMsun) {
            case 1:
            case -1:
...
function _getSumB(T) {
    ...
    var E = 1 - 0.002516 * T - 0.0000074 * Math.pow(T, 2);
    var sumB = -2235 * Math.sin((0, angleCalc_1.deg2rad)(L))
    ...
    calculations_1.MOON_ARGUMENTS_B.forEach(function (args) {
        var argD = args[0];
        var argMsun = args[1];
        var argMmoon = args[2];
        var argF = args[3];
        var argSumB = args[4];
        var tmpSumB = Math.sin((0, angleCalc_1.deg2rad)(argD * D + argMsun * Msun + argMmoon * Mmoon + argF * F));
        switch (argMsun) {
            case 1:
            case -1:
...
function getOpticalLiberationInLongitude(longitude, latitude, T) {
    var latRad = (0, angleCalc_1.deg2rad)(latitude);
    var i = 1.54242;
    var iRad = (0, angleCalc_1.deg2rad)(i);
    var phi = calculations_2.earthCalc.getNutationInLongitude(T);
    var F = getArgumentOfLatitude(T);
    var Omega = getMeanLongitudeOfAscendingNode(T);
    var W = (0, angleCalc_1.normalizeAngle)(longitude - phi - Omega);
